...
export class AuthService {
  ...
  async signUp(dto: SignUpDto): Promise<{ accessToken: string; refreshToken: string }> {
    const { email, password, name, dateNaissance, address,status, telephone, role } = dto;
  ...
  async createAdmin(dto: SignUpDto) {
    ...
    if (role !== 'admin') {
      throw new BadRequestException('Only role "admin" is allowed here');
    }

    const existingUser = await this.userModel.findOne({ email }).exec(); // Add email check
    if (existingUser) {
      throw new BadRequestException('Email already exists');
    }
...
async login(dto: LoginDto): Promise<{ accessToken: string; refreshToken: string }> {
  ...
  const refreshToken = await this.generateRefreshToken(user._id);
  const accessToken = this.jwtService.sign(payload, { expiresIn: '1h' });
  console.log('Generated token:', this.jwtService.decode(accessToken));
  return { accessToken, refreshToken };
}

async refresh(user: { id: string }, refreshToken: string): Promise<{ accessToken: string; refreshToken: string }> {
  ...
  const payload: JwtPayload = { id: user.id, email: dbUser.email, role: dbUser.role };
